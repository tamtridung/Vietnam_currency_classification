...
data_dir = '/home/tamtran/Desktop/wk8_VN_currency_classification/data'
...
# Load model:
final_model = tf.keras.models.load_model('/home/tamtran/Documents/GitHub/VN_currency_classification/model/mobilenetv2_v2.h5')
...
def prepare_learning_data(data_dir, class_names, true_label, aug_dir, number_of_aug_image):
    # make augmentation folder balance:
    # randomly choice 50 images from others class --> copy to augmentated folders
    # --> the augmentated label folder will move 50-numberofaugimg to folder
    from distutils.dir_util import copy_tree
    import os, random, glob
    import shutil

    # random.choices(range(0,))
    for name in class_names:
        # get all files in class folder:
        class_dir = os.path.join(data_dir, name)

        # get list of files in class foler:
        list_files = glob.glob(class_dir+'/*.*')
        if name != true_label:        
            # choose randomly 50 files
            rd_list_file = random.choices(list_files, k = 30)

            for file_path in rd_list_file:
                destination_dir = os.path.join(aug_dir, name)
                shutil.copy(file_path, destination_dir)
        elif name == true_label:
            rd_list_file = random.choices(list_files, k = 30-number_of_aug_image)

            for file_path in rd_list_file:
                destination_dir = os.path.join(aug_dir, name)
                shutil.copy(file_path, destination_dir)

        print(f'class {name} already made')
...
col111, col222 = st.beta_columns((3,1))
with col111:
    st.title('VIETNAM DONG CLASSIFIER')
    st.write('Make by TAM TRAN')

with col222:
    st.image('/home/tamtran/Documents/GitHub/VN_currency_classification/streamlit/vnd_icon.png')
    ...
    with ll_expander:
        ...
        if learn_bt:
            if source_prd == 'Uploaded image':
                ...
                # Prepare data for learning:
                prepare_learning_data(data_dir=data_dir,
                                    class_names=class_names, 
                                    true_label=true_label,
                                    aug_dir=aug_dir,
                                    number_of_aug_image=aug_imgs)
                st.text(f'OK! Data have prepared!')
                ...
                history = final_model.fit(aug_ds, epochs=1)
                st.text(f'OK! Model have learnt new money!')

                # Save update model and delete files
                after_fitting_model(final_model, aug_dir)
